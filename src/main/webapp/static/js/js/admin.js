	...
	function build_emps_table(data) {
		...
		$.each(
						emps,
						function(index, item) {
							var checkBoxTd = $("<th><input type='checkbox' class='check_item' value='"+item.id+"'/></th>");
							var empIdTd = $("<th></th>").append(item.id);
							var empNameTd = $("<th></th>").append(item.username);
							var genderTd = $("<th></th>").append(item.password/* == 'M' ? "男" : "女"*/);
							var emailTd = $("<th></th>").append(item.status);
