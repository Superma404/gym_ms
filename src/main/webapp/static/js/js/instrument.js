	...
	function build_emps_table(data) {
		...
		$.each(
						emps,
						function(index, item) {
							var checkBoxTd = $("<th><input type='checkbox' class='check_item' value='"+item.insId+"'/></th>");
							var empIdTd = $("<th></th>").append(item.insId);
							var empNameTd = $("<th></th>").append(item.insName);
							var genderTd = $("<th></th>").append(item.insUse/* == 'M' ? "男" : "女"*/);
							var emailTd = $("<th></th>").append(item.insStatus);
