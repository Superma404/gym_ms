	...
	//页面显示
	function to_page(pn) {
		$.post("http://localhost:8080/types", {
			"pn" : pn
		}, function(data) {
	...
	function build_emps_table(data) {
		...
		$.each(
						emps,
						function(index, item) {
							var checkBoxTd = $("<th><input type='checkbox' class='check_item' value='"+item.tId+"'/></th>");
							var empIdTd = $("<th></th>").append(item.tId);
							var empNameTd = $("<th></th>").append(item.tStyle);
