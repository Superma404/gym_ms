	...
	function build_emps_table(data) {
		...
		$.each(
						emps,
						function(index, item) {
							var checkBoxTd = $("<th><input type='checkbox' class='check_item' value='"+item.noId+"'/></th>");
							var empIdTd = $("<th></th>").append(item.noId);
							var empNameTd = $("<th></th>").append(item.noName);
							var genderTd = $("<th></th>").append(item.noContent/* == 'M' ? "男" : "女"*/);
							var emailTd = $("<th></th>").append(item.noTime);
							var deptNameTd = $("<th></th>").append(item.noAuthor);
	...
	$(function() {
		...
		$("#emp_save_btn").click(
				function() {
					...
					//将模态框内的表单信息提交给服务器保存
					$.post("http://localhost:8080/userAdd", $("#empAddModel form").serialize(),
							function(data) {
						          //后台验证
								  if(data.state==100){
									  ...
									  if(data.empName!=undefined){
										  show_validate_add_form("#empName_add","fail",data.empName);
									  }
									  if(data.email!=undefined){
										  show_validate_add_form("#email_add", "fail",data.email); 
									  }
						  }
	...
	$(function() {
		...
		$("#emp_del_btn").click(function() {
			...
			$.ajax({
				type : 'POST',
				url : 'http://localhost:8080/usersDel',
