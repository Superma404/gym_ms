	...
	function build_emps_table(data) {
		...
		$.each(
						emps,
						function(index, item) {
							var checkBoxTd = $("<th><input type='checkbox' class='check_item' value='"+item.couId+"'/></th>");
							var empIdTd = $("<th></th>").append(item.couId);
							var empNameTd = $("<th></th>").append(item.couName);
							var genderTd = $("<th></th>").append(item.couCoach/* == 'M' ? "男" : "女"*/);
							var emailTd = $("<th></th>").append(item.couBegintime);
							var deptNameTd = $("<th></th>").append(item.couAdress);
...
*/
		...
		$("#emp_save_btn").click(
				function() {
					...
					//将模态框内的表单信息提交给服务器保存
					$.post("http://localhost:8080/courseAdd", $("#empAddModel form").serialize(),
							function(data) {
						alert("1111111111")
						          //后台验证
								  if(data.state==100){
	...
	$(function() {
		...
		$("#emp_del_btn").click(function() {
			...
			$.ajax({
				type : 'POST',
				url : 'http://localhost:8080/coursesDel',
