	...
	function build_emps_table(data) {
		...
		$.each(
						emps,
						function(index, item) {
							var checkBoxTd = $("<th><input type='checkbox' class='check_item' value='"+item.meId+"'/></th>");
							var empIdTd = $("<th></th>").append(item.meId);
							var empNameTd = $("<th></th>").append(item.meAuthor);
							var genderTd = $("<th></th>").append(item.meContent/* == 'M' ? "男" : "女"*/);
							var emailTd = $("<th></th>").append(item.meTime);
							var deptNameTd = $("<th></th>").append(item.meStatus);
