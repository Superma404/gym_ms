	...
	function build_emps_table(data) {
		...
		$.each(
						emps,
						function(index, item) {
							var checkBoxTd = $("<th><input type='checkbox' class='check_item' value='"+item.usId+"'/></th>");
							var empIdTd = $("<th></th>").append(item.usId);
							var empNameTd = $("<th></th>").append(item.usName);
							var genderTd = $("<th></th>").append(item.usRealname/* == 'M' ? "男" : "女"*/);
							var emailTd = $("<th></th>").append(item.usTel);
							var deptNameTd = $("<th></th>").append(item.usStatus);
