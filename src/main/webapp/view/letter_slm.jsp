                	...
                	<table class="table table-hover">
                		...
                		<tbody>
                			<c:forEach items="${page.list }" var="pro">
                				<tr id=${pro.leId }>
                					<td>${pro.leId }</td>
                					<td>${pro.leUsAddresserid }</td>
                					<td>${pro.leUsRecipientsid }</td>
                					<td>${pro.leContent }</td>
                					<td>
									<button class="btn btn-danger btn-sm" name="btn" leId=${pro.leId }>
			...
			<div class="col-sm-6 col-sm-offset-3">
				<nav aria-label="Page navigation">
					<ul class="pagination pagination-sm">
						<li><a href="selectLetter?pn=1">首页</a></li>
							<li>
								<a href="selectLetter?pn=${page.pageNum-1}"
									aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
								</a>
							</li>
						<c:forEach items="${page.navigatepageNums }" var="n">
								<li><a href="selectLetter?pn=${n }">${n }</a></li>
						</c:forEach>
							<li><a href="selectLetter?pn=${page.pageNum+1 }"
								aria-label="Next"> <span aria-hidden="true">&raquo;</span>
							</a></li>
						<li><a href="selectLetter?pn=${page.pages}">末页</a></li>
					</ul>
				</nav>
		      </div>
                </li>
...
<script type="text/javascript">
		 $(document).ready(function(){
				$(":button[name='btn']").click(function(){
					...
					$.post("deleteLetter",{"leId":leId},function(msg){
						if(msg==1){
